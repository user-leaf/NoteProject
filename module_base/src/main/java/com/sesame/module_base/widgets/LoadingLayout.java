package com.sesame.module_base.widgets;
...
import com.sesame.module_base.R;


/**
 * create by Weavey,Aron
 * on date 2016-03-12
 */
public class LoadingLayout extends FrameLayout {
    ...
    private RelativeLayout errorReloadGroupBtn;
    private TextView networkReloadBtn;

    private View contentView;
    private OnReloadListener listener;
    private onEmptyLoadListener mEmptyLoadListener;
    ...
    public LoadingLayout(Context context, AttributeSet attrs) {
        ...
        showContent = a.getBoolean(R.styleable.LoadingLayout_showContent, false);
        state = a.getInt(R.styleable.LoadingLayout_status, Success);
        pageBackground = a.getColor(R.styleable.LoadingLayout_pageBackground, getColor(mContext, R.color.bg_common_color));
        a.recycle();
    }
    ...
    private void build() {
        ...
        if (mConfig.loadingView == null) {
            ...
            mLottieAnimationView = loadingPage.findViewById(R.id.lavLoad);
        } else {
        ...
        emptyBtn = findViewById(emptyPage, R.id.empty_btn);
        errorReloadBtn = findViewById(errorPage, R.id.error_reload_btn);
        errorReloadGroupBtn = findViewById(errorPage, R.id.error_reload_btn_group);
        ...
        errorReloadGroupBtn.setOnClickListener(v -> {
            if (listener != null) {
                listener.onReload(v);
            }
        });
        ...
        emptyBtn.setOnClickListener(v -> {
            if (mEmptyLoadListener != null) {
                mEmptyLoadListener.onEmptyListener(v);
            }
        });
    ...
    public void setStatus(@Flavour int status) {
        ...
        switch (status) {
            ...
            case Empty:
                ...
                emptyBtn.setVisibility(emptyBtn.getText().toString().isEmpty() ? GONE : VISIBLE);
                errorPage.setVisibility(View.GONE);
                networkPage.setVisibility(View.GONE);
                if (defineLoadingPage != null) {
                    defineLoadingPage.setVisibility(View.GONE);
    ...
    public LoadingLayout setEmptyButtonText(@NonNull String text) {
     ...
     * @return
     */
    public LoadingLayout setErrorReloadBtn(boolean showTextBtn) {
        errorReloadBtn.setVisibility(showTextBtn ? VISIBLE : GONE);
        errorReloadGroupBtn.setVisibility(showTextBtn ? GONE : VISIBLE);
        return this;
    }
    ...
    public LoadingLayout setEmptyLoadListener(onEmptyLoadListener emptyLoadListener) {
        mEmptyLoadListener = emptyLoadListener;
        return this;
    }
    ...
    public LoadingLayout setLoadingPage(@LayoutRes int id) {
        ...
        mLottieAnimationView = view.findViewById(R.id.lavLoad);
        defineLoadingPage = view;
        defineLoadingPage.setVisibility(View.GONE);
        this.addView(view);
        return this;
